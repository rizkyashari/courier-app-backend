...
func SetupSuitePayment(t *testing.T) *SuitePayment {
	s := &SuitePayment{}
	var err error
	...
	s.paymentRepo = NewPaymentRepository(s.GormDB)
	return s
}

func TestGetAllPayments(t *testing.T) {
	suite := SetupSuitePayment(t)

	query := `
	SELECT * FROM "payments" 
	`

	user := sqlmock.NewRows([]string{"id", "user_id", "payment_status", "total_cost", "promo_id"}).
		AddRow(1, 2, 3, 4, 5)

	suite.mock.ExpectQuery(regexp.QuoteMeta(query)).WithArgs(1).WillReturnRows(user)

	data := suite.paymentRepo.AllPayments()
